...
//check if current process is (simulated) root in any communicator. Implemented but currently not used => just return
 ...
 *
 * This is a collective function. All (alive) ranks of the communicator have to call it in order to continue.
 * The call only gets canceled if NextOp is Deafult and the communicator is revoked! For calls like Shrink or
 * Finalize, the function MUST be called by all alive processes, the application needs to make sure of it,
...
int simft::Sim_FT_Check_dead_processes(simft::Sim_FT_MPI_Comm f_comm, simft::NextOp option){
	...
	//if comm is revoked and NextOp is Default, return error (no sync operation is needed in this special case)
	...
	//If comm is revoked, the sync messages have to be sent via a special revoke tag. Only messages other than Default can be sent this way.
	...
	if(f_comm->comm_rank == f_comm->Root_Rank){
		...
		//Attach the information, which operation is next (Shrink, Finalize or normal blocking collective)
		...
		if(f_comm->Dead_Processes_Root.size() > 0){
			...
			Send_Vector.insert(Send_Vector.end(), f_comm->Dead_Processes_Root.begin(), f_comm->Dead_Processes_Root.end());

			//if next operation is MPI_Comm_free, attach NBC_Vector_Send
			if(option == simft::NextOp::Comm_free)
				Send_Vector.insert(Send_Vector.end(), f_comm->NBC_Vector_Send.begin(), f_comm->NBC_Vector_Send.end());
			...
			//f_comm->Blocking_Collective_Count++; //TODO: not needed, there will be only one cycle per comm (err after cycle complete)
			return Ret;
		}else{
			...
			simft::Sim_FT_Custom_Dead_bcast(&Send_Vector, f_comm, revokedAck); //empty vector (no dead processes)
			//f_comm->Recent_Icollectives.clear(); //after a blocking collective, clear the vector at all ranks
			//f_comm->Recent_Icollectives.reserve(10);

			if(f_comm->dead_nodes.size() != f_comm->dead_set.size()){
				f_comm->dead_set = std::set<int> (f_comm->dead_nodes.begin(), f_comm->dead_nodes.end()); //"quite" inefficient (O(n logn)) but only occurs if processes failed
			}
		...
		if(Bcast_Vector.size() > 0){
			if(Bcast_Vector[1] > 0){
				...
				if((unsigned int)Bcast_Vector[1] != f_comm->dead_nodes.size()){
					f_comm->dead_nodes = std::vector<int>(&Bcast_Vector[2],&Bcast_Vector[Bcast_Vector[1]+2]); //+1
					f_comm->Ack_failed_processes = false; //new failure detected => reset ack
				}
				//f_comm->Blocking_Collective_Count++; //TODO: not needed, there will be only one cycle per comm (err after cycle complete)
			}else{
			...
			if(Bcast_Vector[0] == 9){ //comm is revoked, but we started this op while it wasn't revoked => set revoked now and return error
			...
			if(Bcast_Vector.size() > (unsigned int)Bcast_Vector[1]+2){
				f_comm->NBC_Vector_Send = std::vector<int>(&Bcast_Vector[Bcast_Vector[1]+2],&Bcast_Vector[Bcast_Vector.size()]);
			}
...
bool simft::Sim_FT_Receive_dead_msgs_root(simft::Sim_FT_MPI_Comm f_comm){
	...
	if(Dead_Flag == 1){
		...
		int NBC = 0;
		MPI_Recv(&NBC, 1, MPI_INT, Dead_Status.MPI_SOURCE, SIM_FT_DEAD_NBC_TAG, f_comm->c_comm_copy, MPI_STATUS_IGNORE);
		...
		f_comm->CurrentMinNBC_Modified = true; //tells root to initiate a NBC broadcast (to inform comm about new dead processes and NBC count)
		return true;
	}else{
		return false;
	}
 ...
 * so that background messages such as revoke-msg or NBC-msg can be forwarded through the whole communicator.
 * If current rank is root, the function also receives dead-msgs and initiates NBC-broadcst from time to time.
*/
void simft::Sim_FT_Perform_background_operations(simft::Sim_FT_MPI_Comm f_comm, bool inside_Check_dead){
	if(f_comm->Root_Rank == f_comm->comm_rank){
		...
		int PFlag = 0;
		MPI_Iprobe(f_comm->Root_Rank, SIM_FT_NBC_REQUEST_TAG, f_comm->c_comm_copy, &PFlag, MPI_STATUS_IGNORE);
		if(PFlag == 1){
			MPI_Recv(0, 0, MPI_INT, f_comm->Root_Rank, SIM_FT_NBC_REQUEST_TAG, f_comm->c_comm_copy, MPI_STATUS_IGNORE);
			...
			MPI_Isend(&NBCVec[0], NBCVec.size(), MPI_INT, f_comm->Root_Rank, SIM_FT_NBC_RESPONSE_TAG, f_comm->c_comm_copy, &NBC_Request);
	...
	/*
	 * this check is necessary for consistency. Otherwise a revoke broadcast could overtake a Check_dead_processes call
	 * resulting in some processes using the normal tags and some the special revoked tags
	*/
	if(!inside_Check_dead){
		//forward revoke broadcast if one is active (non-root) or initiate if revoke message is received (root)
	...
	if(f_comm->comm_revoked){
		...
		if(Alive_Flag == 1){
			MPI_Recv(&Alive_Status_Recv, 1, MPI_CHAR, Alive_Status.MPI_SOURCE, Alive_Status.MPI_TAG, f_comm->c_comm_copy_p2p, MPI_STATUS_IGNORE);
			if(Alive_Status_Recv == I_AM_ALIVE || Alive_Status_Recv == I_AM_ALIVE_NB){ //should always be true!!! (TODO: maybe remove if???)
		...
		if(Recv_Flag == 1){
			...
			MPI_Recv(&Receive_Vector[0], count, MPI_INT, Recv_Status.MPI_SOURCE, SIM_FT_REDUCE_TAG, f_comm->c_comm_copy_coll, MPI_STATUS_IGNORE);
			...
			MPI_Isend(&Send_Vector[0], 1, MPI_INT, Recv_Status.MPI_SOURCE, SIM_FT_DEAD_TAG, f_comm->c_comm_copy_coll, &Send_Request);
			MPI_Request_free(&Send_Request);
		}
	}
}

/* Special: perform background operations on all active communicators (possibly slowing down if too many active communicators)
 * */
...
void simft::Sim_FT_Perform_background_operations_dead(simft::Sim_FT_MPI_Comm f_comm){
	if(f_comm->Root_Rank == f_comm->comm_rank){
		...
		while(simft::Sim_FT_Receive_dead_msgs_root(f_comm));
		simft::Sim_FT_NBC_bcast_root(f_comm);
		if((int)simft::Sim_FT_MPI_COMM_WORLD->Dead_Processes_Root.size() == simft::Sim_FT_MPI_COMM_WORLD->comm_size){
			...
			Send_Vector.insert(Send_Vector.end(), simft::Sim_FT_MPI_COMM_WORLD->Dead_Processes_Root.begin(), simft::Sim_FT_MPI_COMM_WORLD->Dead_Processes_Root.end());

			simft::Sim_FT_Custom_Ibcast_Root(&Send_Vector, &(simft::Sim_FT_MPI_COMM_WORLD->Dead_Buf), Send_Vector.size(), MPI_INT, SIM_FT_DEAD_TAG, simft::Sim_FT_MPI_COMM_WORLD->c_comm_copy_coll, simft::Sim_FT_MPI_COMM_WORLD);
	...
	if(Status_Flag == 1){
		char Rec_Char;
		MPI_Recv(&Rec_Char, 1, MPI_CHAR, Status_Status.MPI_SOURCE, Status_Status.MPI_TAG, f_comm->c_comm_copy_p2p, MPI_STATUS_IGNORE);
		if(Rec_Char == I_AM_ALIVE){ //I_AM_ALIVE_NB will not be responded! Instead, the alive process will know about the dead process after receiving nbc bcast
			//Respond alive-message with dead-message
			char DeadMsg = I_AM_DEAD;
			MPI_Send(&DeadMsg, 1, MPI_CHAR, Status_Status.MPI_SOURCE, Status_Status.MPI_TAG, f_comm->c_comm_copy_p2p);
		}
	}
	/* ########################*/
...
void simft::Sim_FT_NBC_bcast_root(simft::Sim_FT_MPI_Comm f_comm){
	if(f_comm->CurrentMinNBC_Modified){
		if(f_comm->LastNBC_bcast + SIM_FT_WAIT_NBC_BCAST < MPI_Wtime()){
			...
			Send_Vector.insert(Send_Vector.end(), f_comm->root_recent_dead_set.begin(), f_comm->root_recent_dead_set.end());
			f_comm->root_recent_dead_set.clear();

			simft::Sim_FT_Custom_Ibcast_Root(&Send_Vector, &Buffer_Vector, Send_Vector.size(), MPI_INT, SIM_FT_BCAST_NBC_TAG, f_comm->c_comm_copy, f_comm);
...
int simft::Sim_FT_NBC_bcast_Nroot(simft::Sim_FT_MPI_Comm f_comm){
	...
	std::vector<int> Buffer_Vector;
	if(simft::Sim_FT_Custom_Ibcast_NRoot(&NBC_Count, &Buffer_Vector, &Count, MPI_INT, SIM_FT_BCAST_NBC_TAG, f_comm->c_comm_copy, f_comm)){
...
void simft::Sim_FT_Perform_nb_operations(std::vector<simft::Sim_FT_Istats> * NB_ops_to_perform, simft::Sim_FT_MPI_Comm f_comm){
#ifndef DISABLE_NBC
	if(f_comm->Recent_Icollectives.size() < NB_ops_to_perform->size()){ //else: no NB Collective to be done
			...
			switch((*NB_ops_to_perform)[i].Type){
				...
				case 1: //ibcast
				{
					if((*NB_ops_to_perform)[i].Datatype == 0){//MPI_CHAR
						...
						MPI_Ibcast(&(*NB_ops_to_perform)[i].BuffChar.front(), (*NB_ops_to_perform)[i].count, MPI_CHAR, (*NB_ops_to_perform)[i].root, f_comm->c_comm, &(*NB_ops_to_perform)[i].request->c_request);
						...
						//begin Ibcast with dummy Array to free other nodes with active Ibcasts
						MPI_Ibcast(&(*NB_ops_to_perform)[i].BuffInt.front(), (*NB_ops_to_perform)[i].count, MPI_INT, (*NB_ops_to_perform)[i].root, f_comm->c_comm, &(*NB_ops_to_perform)[i].request->c_request);
						...
						//begin Ibcast with dummy Array to free other nodes with active Ibcasts
						MPI_Ibcast(&(*NB_ops_to_perform)[i].BuffDouble.front(), (*NB_ops_to_perform)[i].count, MPI_DOUBLE, (*NB_ops_to_perform)[i].root, f_comm->c_comm, &(*NB_ops_to_perform)[i].request->c_request);
						...
						std::cout << "SIM_FT FATAL ERROR: In simft::Sim_FT_Perform_nb_operations, ibcast: Datatype not yet implemented - " << (*NB_ops_to_perform)[i].Datatype << "\n";
						exit(-1);
					}
				...
				case 2: //iallreduce
				{
					if((*NB_ops_to_perform)[i].Datatype == 0){//MPI_CHAR
						...
						std::cout << "SIM_FT FATAL ERROR: In simft::Sim_FT_Perform_nb_operations, iallreduce: Datatype not implemented yet - " << (*NB_ops_to_perform)[i].Datatype << "\n";
						exit(1);
					}
					break;
				}
				...
				default:
					std::cout << "SIM_FT FATAL ERROR: In simft::Sim_FT_Perform_nb_operations: Function Type not implemented - " << (*NB_ops_to_perform)[i].Type << "\n";
					exit(-1);
			}
		}
	}
...
void simft::Sim_FT_Complete_nb_operations(std::vector< simft::Sim_FT_Istats > *Deserialized_NBC_Vector, simft::Sim_FT_MPI_Comm f_comm){
	 ...
	 */
	for(unsigned int i = f_comm->Recent_Icollectives.size(); i < (*Deserialized_NBC_Vector).size(); i++){
	...
	for(unsigned int i = f_comm->Recent_Icollectives.size(); i < (*Deserialized_NBC_Vector).size(); i++){
		 ...
		 * */
//		if((*Deserialized_NBC_Vector)[i].BuffChar != nullptr)delete[] (*Deserialized_NBC_Vector)[i].BuffChar;
//		if((*Deserialized_NBC_Vector)[i].BuffChar2 != nullptr)delete[] (*Deserialized_NBC_Vector)[i].BuffChar2;
//		if((*Deserialized_NBC_Vector)[i].BuffInt != nullptr)delete[] (*Deserialized_NBC_Vector)[i].BuffInt;
//		while(1);
...
bool simft::Sim_FT_Check_comm_revoked(simft::Sim_FT_MPI_Comm f_comm){
	...
	if(current_rank == f_comm->Root_Rank){
		...
		if(f_comm->comm_revoked == true){
			//if another rank decides to revoke comm, receive message and "discard" it to free the resource
			int Revoke_Flag = 0;
			MPI_Status Revoke_Status;
			MPI_Iprobe(MPI_ANY_SOURCE, SIM_FT_REVOKE_TAG, f_comm->c_comm_copy, &Revoke_Flag, &Revoke_Status);
			if(Revoke_Flag == 1){
			...
			if(Revoke_Flag == 1){
				...
				if(f_comm->comm_revoked == false){
					...
					simft::Sim_FT_Custom_Ibcast_Root(&Send_Vector, &Buffer_Vector, 1, MPI_INT, SIM_FT_BCAST_REVOKE_TAG, f_comm->c_comm_copy, f_comm);
...
//		}else{
			...
			bool BCast_Ret = simft::Sim_FT_Custom_Ibcast_NRoot(&BCast_Revoke_Message, &Buffer_Vector, &Count, MPI_INT, SIM_FT_BCAST_REVOKE_TAG, f_comm->c_comm_copy, f_comm);
			if(BCast_Ret){
				...
				if(BCast_Revoke_Message[0] == 2){
					...
					std::cout << "++####### ERROR RECV MS Msg ="  << BCast_Revoke_Message[0] <<  "bei Rank " << current_rank << "\n";
					return false;
				}
			}else{
				return f_comm->comm_revoked;
...
void simft::Sim_FT_Initialize_new_comm(simft::Sim_FT_MPI_Comm *f_new_comm, bool firstInit){
	...
	//if c_comm is MPI_COMM_NULL, we can't (and mustn't) call MPI_Comm_dup => set f_new_comm to NULL and return
   ...
   (*f_new_comm)->Root_Rank = size-1;
	...
	(*f_new_comm)->Bcast_Successors = simft::Bcast_Get_Successors((*f_new_comm)->Root_Rank, (*f_new_comm)->comm_rank, (*f_new_comm)->comm_size);
	(*f_new_comm)->Bcast_Predecessor = simft::Bcast_Get_Predecessor((*f_new_comm)->Root_Rank, (*f_new_comm)->comm_rank, (*f_new_comm)->comm_size);
...
void simft::Sim_FT_kill_me(){
	//Send current Non-Blocking Collectives count to all root processes - at the same time notify them about dead status
	...
	for( auto Actives : simft::Sim_FT_Current_Active_Communicators){
		...
		MPI_Isend(&NBC, 1, MPI_INT, Actives->Root_Rank, SIM_FT_DEAD_NBC_TAG, Actives->c_comm_copy, &Send_Request);
		MPI_Request_free(&Send_Request);
		...
		if(Actives->comm_revoked){ //comm is revoked, use the alternative tags for reduce and broadcast
	...
	while(1){
		...
		for( auto Actives : simft::Sim_FT_Current_Active_Communicators){
			...
			if( Actives->comm_size == 1){ //if comm_size is 1, no alive process is in there: free comm and delete it
				...
				simft::Sim_FT_Current_Active_Communicators.erase(Actives); //remove freed communicator from active comm set
			...
			if(Actives->Root_Rank != Actives->comm_rank){
				...
				if(Actives->Ireduce_Active && simft::Sim_FT_Custom_Ibcast_NRoot(&buf, &(Actives->Dead_Buf), &count, MPI_INT, Actives->DeadTag, Actives->c_comm_copy_coll, Actives)){
					//Ibcast completed, set Ireduce_Active to false, so that a new reduce operation can be started
					...
					if(buf[0] == 1){ //Shrink next
						...
						//f_comm->NBC_Vector_Send = std::vector<int>(&Bcast_Vector[Bcast_Vector[1]+1],&Bcast_Vector[Bcast_Vector.size()-1]);
						...
						//at this point, switch from normal tag to reduced tag, if comm is revoked or revoke message via broadcast is recieved
						Actives->ReduceTag = SIM_FT_REDUCE_TAG_REVOKED;
						Actives->DeadTag = SIM_FT_DEAD_TAG_REVOKED;
					}
				}
				...
				if(simft::Sim_FT_Custom_Ireduce(Actives, true)){
					...
					std::vector<int> Send_Vector; //[0]: NextOp; [1]: Dead processes count; [>1]: Dead processes id's
					...
					if(Actives->Last_Send_Vector[3] == 3){
						 ...
						 */
						MPI_Request NBC_Request;
						MPI_Isend(0, 0, MPI_INT, Actives->CurrentMaxNBC_Rank, SIM_FT_NBC_REQUEST_TAG, Actives->c_comm_copy, &NBC_Request);
						MPI_Request_free(&NBC_Request);

						MPI_Status RStatus;
						int PFlag = 0;
						while(PFlag == 0){
							MPI_Iprobe(Actives->CurrentMaxNBC_Rank, SIM_FT_NBC_RESPONSE_TAG, Actives->c_comm_copy, &PFlag, &RStatus);
						...
						MPI_Recv(&Actives->NBC_Vector_Send[0], Count, MPI_INT, Actives->CurrentMaxNBC_Rank, SIM_FT_NBC_RESPONSE_TAG, Actives->c_comm_copy, MPI_STATUS_IGNORE);
						// ####################
					...
					//Attach the information, which operation is next (Shrink, Finalize, Comm_free, Comm_agree or normal blocking collective)
					...
					Send_Vector.insert(Send_Vector.end(), Actives->Dead_Processes_Root.begin(), Actives->Dead_Processes_Root.end());


					// attach NBC_Vector_Send to our Send_Vector
					if(Send_Vector[0] == 3){
						Send_Vector.insert(Send_Vector.end(), Actives->NBC_Vector_Send.begin(), Actives->NBC_Vector_Send.end());
					}

					simft::Sim_FT_Custom_Ibcast_Root(&Send_Vector, &(Actives->Dead_Buf), Send_Vector.size(), MPI_INT, Actives->DeadTag, Actives->c_comm_copy_coll, Actives);
...
//if custom IBcast returns true, buf is received and can be read OR current rank is root; in this case buf should not be touched!
bool simft::Sim_FT_Custom_Ibcast_Root(std::vector<int> *buf_out, std::vector<int> *buf_copy, int count, MPI_Datatype datatype, int tag, MPI_Comm comm, simft::Sim_FT_MPI_Comm f_comm){
	...
	for(unsigned int i = 0; i < f_comm->Bcast_Successors.size(); i++){
		MPI_Request Send_Request; //will never be read again because we assume that eventually all Isend operations finish successfully
		if(count == 0){
			MPI_Isend(0, 0, datatype, f_comm->Bcast_Successors[i], tag, comm, &Send_Request);
		}else{
			MPI_Isend(&buf_copy->front(), count, datatype, f_comm->Bcast_Successors[i], tag, comm, &Send_Request);
		}
		MPI_Request_free(&Send_Request);
	}
	return true;
...
bool simft::Sim_FT_Custom_Ibcast_NRoot(std::vector<int> *buf_out, std::vector<int> *buf_copy, int *count, MPI_Datatype datatype, int tag, MPI_Comm comm, simft::Sim_FT_MPI_Comm f_comm){
	int Bcast_Flag = 0;
	...
	if(Bcast_Flag == 1){
		...
		if(sendcount == 0){
			...
			MPI_Recv(&buf_out->front(), sendcount, datatype, Bcast_Status.MPI_SOURCE, tag, comm, MPI_STATUS_IGNORE);
			*buf_copy = *buf_out; //set copy-buffer to out-buffer
		}
		...
		for(unsigned int i = 0; i < f_comm->Bcast_Successors.size(); i++){
			...
			if(sendcount == 0){
				...
				MPI_Isend(&buf_copy->front(), sendcount, datatype, f_comm->Bcast_Successors[i], tag, comm, &Send_Request);
			}
			MPI_Request_free(&Send_Request);
		}
		return true;
...
void simft::Sim_FT_Custom_Dead_bcast(std::vector<int> *buf, simft::Sim_FT_MPI_Comm f_comm, bool revoked){
	...
	if(f_comm->comm_rank == f_comm->Root_Rank){
		...
		simft::Sim_FT_Custom_Ibcast_Root(buf, &(f_comm->Dead_Buf), buf->size(), MPI_INT, BcastTag, f_comm->c_comm_copy_coll, f_comm);

	}else{
		int count = 0;

		while(!simft::Sim_FT_Custom_Ibcast_NRoot(buf, &(f_comm->Dead_Buf), &count, MPI_INT, BcastTag, f_comm->c_comm_copy_coll, f_comm)){
			simft::Sim_FT_Perform_background_operations( true );
		}
	}
	simft::Sim_FT_Perform_background_operations( true );
...
void simft::customReduceOp( void *inV, void *inoutV, int *len, MPI_Datatype *dptr )
{
	...
	if(*(inout + 1) < *(in + 1)){ //check, if NBC count and id has to be updated (we want the max. NBC count and its proc id)
		*(inout + 1) = *(in + 1);
		*(inout + 2) = *(in + 2);
	}
	*inout = *inout + *in; //add dead count from successor to local dead count
	if(*(inout + 3) == 0){
		...
		*(in + 3) = *(inout + 3); //comm_agree performs "and"-operation => if local value is true, set it to the remote value (true or false)
	}
}

//Reduce Vector has the following form: 4x int: ( dead count; max. NBC; rank max. NBC; NextOp )
int simft::Sim_FT_Custom_Reduce(int Send_Vector[], simft::Sim_FT_MPI_Comm f_comm, simft::NextOp option, bool Revoked){
...
//#ifndef USE_NON_BLOCKING_COLLECTIVES
	...
	Send_Vector[3] = (int)option; //if the next operation shall be Shrink or Finalize etc., this must be propagated through the comm to inform the dead processes about it
			...
			if(!Received[i]){
				...
				if(Recv_Flag == 1){
					...
					if(count == 0){ //"deprecated", should not happen, because size of reduce msg is at least 3 int!
						...
						MPI_Recv(0, count, MPI_INT, f_comm->Bcast_Successors[i], ReduceTag, comm, MPI_STATUS_IGNORE);
					}else{
						MPI_Recv(&Receive_Vectors[0], count, MPI_INT, f_comm->Bcast_Successors[i], ReduceTag, comm, MPI_STATUS_IGNORE);
					}
...
//If a process is dead, it needs to continuously forward reduce-messages, as well as add the own dead-message
bool simft::Sim_FT_Custom_Ireduce(simft::Sim_FT_MPI_Comm f_comm, bool proc_dead){ //, std::vector<std::vector<int>> &Receive_Vectors, std::vector<bool> &Received, std::vector<MPI_Request> &Recv_Request){

	//explanation: if a reduce message is already forwarded to the predecessor, a new reduce mustn't be startet until the bcast is recieved and Ireduce_Active is set to false
	...
	if(f_comm->Received_Cnt < f_comm->Bcast_Successors.size()){
		...
		if(f_comm->comm_rank != f_comm->Root_Rank){ //if root, process has to access f_comm->Last_Send_Vector to get all failed processes
			MPI_Request Send_Request;
...
std::vector<int> simft::Bcast_Get_Successors(int root_node, int node_id, int tree_size, int Successor_Count){
	...
	for(int i = 0; i < Successor_Count; i++){
		...
		int Normalized = (simft::ConvertId(simft::NormalizeId(node_id, root_node), tree_size) * Successor_Count + i + 1);
...
int simft::Bcast_Get_Predecessor(int root_node, int node_id, int tree_size, int Successor_Count){
	...
	ReturnInt = DeNormalizeId( (Normalized - 1 - ((Normalized - 1)%Successor_Count)) / Successor_Count, root_node );
	return ReturnInt;
}
...
int simft::NormalizeId(int id, int root){
	return id - root;
}

int simft::DeNormalizeId(int id, int root){
	return id + root;
}
...
std::vector< simft::Sim_FT_Istats > simft::Vector_to_NBC( std::vector<int> NBC_Vec ){
	...
	returnVec.reserve( NBC_Vec.size() / 5 ); //assume our NBC_Vec has the correct form, so we don't check if the size is correct
